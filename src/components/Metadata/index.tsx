...
const Metadata = ({
  ...
  const scanLink = `https://mumbai.polygonscan.com/token/${process.env.NEXT_PUBLIC_NODE_ADDRESS}?a=${node.tokenId}`;
  return (
      ...
      <tbody>
        ...
        <tr className="border-b border-color border-dashed">
          ...
          <td className="px-2 py-4 text-right whitespace-nowrap truncate">
            <p>
              {truncate(process.env.NEXT_PUBLIC_NODE_ADDRESS, 30)}
            </p>
          </td>
          <td
            onClick={() => copyToClipboard(process.env.NEXT_PUBLIC_NODE_ADDRESS)}
