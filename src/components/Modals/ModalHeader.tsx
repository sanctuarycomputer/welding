...
const ModalHeader: FC<Props> = ({ title, hint, onClickClose, onClickBack }) => {
  return (
    <>
      <div
        className={`${bg} fixed sm:relative w-full flex p-4 border-b justify-between z-10`}
      >
        <div>
          {onClickBack && (
            <p
      ...
      {/* This is a spacer */}
      <div
        className={`relative sm:hidden w-full flex p-4 border-b justify-between opacity-0`}
      >
        <div>
          {onClickBack && (
            <p
