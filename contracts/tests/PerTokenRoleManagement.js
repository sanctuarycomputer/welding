...
describe("Behavior: Per-token Role Management", function () {
  ...
  it("can manage roles on a per-token basis", async function () {
    ...
    await expect(
      contract.connect(addr2).makeRevision(docId, '789')
    ).to.be.revertedWith("insufficient_permissions");

    /* Addr1 gives Addr2 the EDITOR_ROLE */
    expect(await contract.connect(addr1).getRoleMemberCount(docId, EDITOR_ROLE)).to.equal(0);
    ...
    expect(await contract.connect(addr1).getRoleMemberCount(docId, EDITOR_ROLE)).to.equal(1);

    /* Roles are enumerable too, so we can list role holders */
    expect(await contract.getRoleMemberCount(docId, EDITOR_ROLE)).to.equal(1);
    expect(await contract.getRoleMember(docId, EDITOR_ROLE, 0)).to.equal(addr2.address);

    /* Addr2 can now revise the NFT... */
    await contract.connect(addr2).makeRevision(docId, '789');
